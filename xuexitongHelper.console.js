...
(function(){
    ...
    loadJQ(function(){
        window.app={
            ...
            run:function(){
                this._logPhase("学习通助手","控制台版 - URL: "+location.href.substring(0,80));
                ...
                if(pt==='course_list'){this._logPhase("学习通助手","课程列表页跳过");return}
                if(pt==='chapter_list'){this._logPhase("学习通助手","章节列表→自动进入");if(inTop){this._createControlPanel();this._createDebugPanel()}this._runChapterListAuto();return}
                ...
                var doDetect=function(attempt){
                    ...
                    self._logPhase("学习通助手","媒体类型: "+self.configs.mediaType+" (尝试"+(attempt+1)+")");
                    if(self.configs.mediaType==='unknown'&&attempt<10){
                        self._logPhase("学习通助手","媒体未就绪，2秒后重试("+(attempt+1)+"/10)");
                    ...
                    else{self._logPhase("学习通助手","重试10次后仍未知，走音频兜底");self._runContentPageAudio()}
                };
                setTimeout(function(){doDetect(0)},500)
            },

            _detectPageType:function(){var u=window.location.pathname,ul=window.location.href;if(ul.includes('i.chaoxing.com/base')||u.includes('/studyApp/'))return'course_list';if(u.includes('/mycourse/studentcourse'))return'chapter_list';if(u.includes('/mycourse/studentstudy'))return'study_page';if(u.includes('/knowledge/cards'))return'content_page';return'unknown'},
            ...
            nextUnit:function(){
                if(this.configs.loopMode){this._logPhase("导航","模式2：#right1跳转");var btn=document.getElementById('right1');if(!btn){try{if(window.parent&&window.parent.document!==window.document)btn=window.parent.document.getElementById('right1')}catch(e){}}if(!btn){try{if(window.top&&window.top.document!==window.document)btn=window.top.document.getElementById('right1')}catch(e){}}if(btn){btn.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,view:window}));this._resetState();setTimeout(function(){try{this._initCellData();this._playCurrent()}catch(e){}}.bind(this),3000)}this._clearCheckInterval();return}
            ...
            _startTaskCompletionMonitor:function(){
                var self=this;var obs=new MutationObserver(function(muts){for(var m=0;m<muts.length;m++){if(muts[m].type==='childList'){for(var n=0;n<muts[m].addedNodes.length;n++){var nd=muts[m].addedNodes[n];if(nd.nodeType===Node.ELEMENT_NODE&&nd.querySelectorAll('[aria-label="任务点已完成"]').length>0){if(!self.configs.loopMode)self._navigateToNextSection();return}}}else if(muts[m].type==='attributes'&&muts[m].attributeName==='aria-label'&&muts[m].target.getAttribute('aria-label')==='任务点已完成'){if(!self.configs.loopMode)self._navigateToNextSection();return}}});
            ...
            togglePause:function(){
                ...
                var btn=document.getElementById('fq-pause-btn');
            ...
            _createControlPanel:function(){
                ...
                var setMode=function(m){self.configs.loopMode=m===2;var m1=document.getElementById('fq-mode-1'),m2=document.getElementById('fq-mode-2');if(m1)m1.className='fq-mode-btn'+(m===1?' fq-mode-active':'');if(m2)m2.className='fq-mode-btn'+(m===2?' fq-mode-active':'');self._logPhase("播放模式",m===1?"模式1 顺序":"模式2 全部");if(m===1){var d=self._detectTaskCompleted();try{if(!d&&window.parent&&window.parent.document!==window.document)d=window.parent.document.querySelectorAll('[aria-label="任务点已完成"]').length>0}catch(e){}try{if(!d&&window.top&&window.top.document!==window.document)d=window.top.document.querySelectorAll('[aria-label="任务点已完成"]').length>0}catch(e){}if(d){self._logPhase("播放模式","已完成→跳转");self._navigateToNextSection();return}}if(self.configs.paused){self.togglePause()}else{self._tryResumePlayback("mode-switch")}};
                p.querySelector('#fq-mode-1').addEventListener('click',function(e){e.stopPropagation();setMode(1)});
                p.querySelector('#fq-mode-2').addEventListener('click',function(e){e.stopPropagation();setMode(2)});
                ...
                console.log("%c✓ 学习通助手 - 面板已创建","color:#4CAF50;font-weight:bold");
            },

    ...
    function initializePlayer() {
        window.app = {
            ...
            _startTaskCompletionMonitor() {
                const obs=new MutationObserver((mutations)=>{for(const m of mutations){if(m.type==='childList'){for(const n of m.addedNodes){if(n.nodeType===Node.ELEMENT_NODE&&n.querySelectorAll('[aria-label="任务点已完成"]').length>0){this._navigateToNextSection();return}}}else if(m.type==='attributes'&&m.attributeName==='aria-label'&&m.target.getAttribute('aria-label')==='任务点已完成'){this._navigateToNextSection();return}}});
